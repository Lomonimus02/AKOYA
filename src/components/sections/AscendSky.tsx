...
export function AscendSky() {
  return (
    <section id="ascend" className="ascend-wash scroll-mt-24">
      <div className="flex flex-col lg:grid lg:min-h-dvh lg:grid-cols-[minmax(28rem,38%)_minmax(0,1fr)]">
        ...
        <div className="ascend-panel relative z-10 order-2 -mt-36 flex flex-col justify-between px-6 pt-28 pb-16 md:px-12 md:-mt-44 md:pt-32 md:pb-20 lg:order-1 lg:mt-0 lg:h-full lg:px-12 lg:pt-36 lg:pb-24 xl:px-16">
          <Reveal>
            <SectionCopy
              onDark
              kicker="48"
          ...
          <Reveal delay={0.1}>
            <ul className="mt-12 max-w-lg border-t border-white/15 lg:mt-16">
              {rooms.map((room) => (
                <li key={room.name} className="border-b border-white/10 py-4">
                  <p className="text-[0.68rem] tracking-[0.28em] text-aqua uppercase">
                    {room.name}
                  </p>
                  <p className="mt-2 font-display text-[1.35rem] leading-snug font-light text-white/90 md:text-[1.5rem]">
                    {room.note}
                  </p>
                </li>
              ))}
      ...
      <div className="flex flex-col lg:grid lg:min-h-[90vh] lg:grid-cols-[minmax(0,1.2fr)_minmax(28rem,38%)]">
        <figure className="ascend-terrace relative">
          <img
            ...
            decoding="async"
          />
          <figcaption className="absolute bottom-6 left-6 z-10 text-[0.68rem] tracking-[0.22em] text-white/90 uppercase [text-shadow:0_8px_24px_rgba(20,52,58,0.45)] md:left-8 lg:bottom-8">
            Among the crown
          </figcaption>
        </figure>
